$(document).ready(function() {
    console.log("Vytre sam") // koga da inicializiram application-a
    window.session = {
        'user_bean': {},
    ...
    try {
        ...
        console.log(window.session.user_bean.mail);

        $('#user_bar_mail').text(session.user_bean.mail);
        var arr = session.user_bean.full_name.split(' ');
        $('#Account').text(arr[0] + ' ' + arr[arr.length - 1]).append('<b class="caret"></b>')
    } catch (error) {
...
//    window.session.elements_per_page = parseInt(session.user_bean.items_per_page);
    ...
    //Full View for button highlight
...
//    var el_Change = $('#nav_change');
//    el_Change.on('mouseover', function () {
//        el_Change.addClass('active')
//    })

//    el_Change.on('mouseout', function () {
//        el_Change.removeClass('active')
//    })
    ...
    //List Create buttons events
    $('#List').on('click', function() {
        $('#Create_div').hide();
//        $('#ViewProfile_div').hide();
        viewProfileHide();
        $('#List_div').show();
    })
...
//                params.el.find('#user_name_middle').val(data.mName);
//                params.el.find('#user_name_last').val(data.lName);
...
//        vp_el.find('#user_name').attr('disabled', 'disabled');
...
//        vp_el.find('#user_type').attr('disabled', 'disabled');
    ...
    //BACK BUTTON from view PROFILE

    // SAVE BUTTON FROM view profile
    ...
    // View Projects anchors event attachment
//    $('.anchor-listings-project').on('click', function (e) {
//        var kotva = e.target;
//        var id = $(kotva).parent().prev().text();
//        console.log(id) // get Project ID
//    })
    // View Tasks ancors event attachment
//    $('.anchor-listings-task').on('click', function(e) {
//        var kotva = e.target;
//        var id = $(kotva).parent().prev().text();
//
//        console.log(id); // get Task ID
//    });
    //View User anchors event attachment
...
//        }
        ...
        for (var i = 0; i < arr.length; i++) {
            body_el.append(
                    ...
                    '</td><td><a href="#" class="anchor-listings-project">' + arr[i].pName + //<a href="#" class="anchor-listings-project">
                    '</a></td><td>' + arr[i].pDesc + // </a>
                    '</td><td>' + arr[i].pUsers +
                    '</td><td>' + arr[i].pTasks +
                    '</td></tr>');
        }
        ...
        var pag_ul_el_ctr = Math.ceil(arr.length / parseInt(session.user_bean.items_per_page)) // number of pag elements
        ...
        pag_ul.append('<li class=' + (pag_ul_el_ctr < 2 ? "disabled" : "") + '><a href="#list_projects_table_body" id="list_proj_pag_forw">&raquo;</a></li>') // use find on pag_ul to determine thesse !

        //attach events for paginations
        pag_ul.find('.list_projects_pag').on('click', function(e) {
            var src = $(e.target).parent(); // get LI
    ...
    function renderTasks(json, body_el, pag_ul) {
        ...
        for (var i = 0; i < arr.length; i++) {
            body_el.append('<tr' + (i < parseInt(session.user_bean.items_per_page) ? "" : " hidden=\"hidden\"") +
                    ...
                    '>' + (json.important ? '<td hidden="hidden"><a target="_blank" href="viewImportantTask.jsp?taskID=' +
                            encodeURIComponent(arr[i].tID) + '&projectName=' + arr[i].tProject + '" hidden="hidden" id="shit">zaebi</a></td>' : '') +
                    '<td>' + arr[i].tID +
...
//            console.log(id) // get Project ID
    ...
    function renderSingleComment(from, date, text) {
        return '<div class="my-jumbotron"><p class="text-right"><b>' + from +
                '</b>   <em>' + date +
                '</em></p><p class="text-left">' + text +
                '</p></div>';
    }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.error) {
                    ...
                    alert(data.success);
//                    listSingleTask({
//                        'id': $('#list_tasks_single_form').find('#task_id').val(),
//                        'div_el': $('#list_tasks_single'),
//                        'task_list_div': $('#list_tasks'),
...
//                });
    ...
    // LIST SINGLE PROJECT AJAX CALL
    function listSingleProject(params) {

        console.log(params.id) // get Project ID
...
//                params.el.find('#list_projects_single_edit').trigger('click');
            },
            error: function(dummy, message) {
                console.log('request failed : ' + message + ' emi')
            }
